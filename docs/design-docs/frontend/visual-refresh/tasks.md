...
| `themeManager.js`、`ThemeSwitcher.vue`、`main.js`、`style.css`、页面骨架组件、`tests/theme-manager.test.js` | 新增/修改 | Task 14 | 三套主题、切换交互、设备持久化与测试 |
| `themeManager.js`、`ThemeSwitcher.vue`、`style.css`、`tests/theme-manager.test.js` | 修改 | Task 15 | 赛博朋克深色主题和明确的主题入口文案 |
| `InteractionEffects.vue`、`interactionEffects.js`、`ThemeSwitcher.vue`、`style.css`、`App.vue`、特效测试 | 新增/修改 | Task 16 | 点击星火、主题涟漪和卡片微光 |
...
### 任务 14: [x] 新增三套可持久化页面主题
- 文件: `index.html`、`src/main.js`、`src/style.css`、`src/theme/themeManager.js`、`src/components/ThemeSwitcher.vue`、`src/components/Header.vue`、`src/App.vue`、`src/components/CategoryNav.vue`、`src/components/Dashboard.vue`、`src/components/LinkList.vue`、`src/components/SearchBar.vue`、`tests/theme-manager.test.js`（新增/修改）
- 依赖: Task 13
- spec 映射: 3.1、3.2、4.1、4.2.1、4.2.2、4.2.3、4.2.5、4.3、5、6、7
- 说明: 保留当前“岁序清新”为默认主题，增加“暖杏书卷”和“云岚静蓝”，通过语义 Token 即时切换并在当前设备保存偏好。
- 验收标准:
  - [x] 默认主题与当前正式页面主色一致，三套主题 ID 和元数据稳定
  - [x] Header 可通过键盘和触屏打开主题面板、选择主题、Escape 关闭并恢复焦点
  - [x] 非法存储值或 localStorage 异常回退默认主题且不阻断启动
  - [x] 主题切换不修改业务 Store、快照、账号空间或同步状态
  - [x] 三套主题在首页、资源库、侧栏和主要弹窗均有明确视觉差异且文字可读
  - [x] 1200 × 800 与 390 × 844 无横向溢出，刷新后保持所选主题
  - [x] `npm run build` 与 `npm test` 返回 0
  - [x] Code Review PASS
- 子任务:
  - [x] 14.1: 建立主题元数据、校验、应用、持久化和启动恢复模块
  - [x] 14.2: 实现 Header 主题切换器及桌面/手机交互
  - [x] 14.3: 增加三套语义 Token 并迁移页面骨架主要固定表面
  - [x] 14.4: 增加主题管理单元测试
  - [x] 14.5: 执行代码审查、全量测试和三主题浏览器验收

### 任务 15: [x] 新增霓虹夜航主题并强化主题入口
- 文件: `src/theme/themeManager.js`、`src/components/ThemeSwitcher.vue`、`src/style.css`、`tests/theme-manager.test.js`（修改）
- 依赖: Task 14
- spec 映射: 3.1、3.2、4.1、4.2.1、4.2.2、4.3、5、7
- 说明: 增加深海黑蓝、电光青和霓虹紫构成的赛博朋克深色主题，并让 Header 入口明确表达“主题切换”、选项完整说明各主题风格。
- 验收标准:
  - [x] 主题定义包含稳定 `cyber` ID 和完整风格描述，原三套 ID 与默认主题不变
  - [x] Header 桌面端明确显示“主题切换”和当前主题，移动端具有等价 aria-label
  - [x] 霓虹夜航覆盖全局背景、Header、侧栏、首页卡片、资源库和主要工具弹窗
  - [x] 深色主题正文、次要文字、边框、输入框、按钮及状态提示可读
  - [x] 四套主题键盘切换、持久化、跨标签页更新和异常降级保持正常
  - [x] 1200 × 800 与 390 × 844 下主题面板和页面无横向溢出
  - [x] `npm run build` 与 `npm test` 返回 0
  - [x] Code Review PASS
- 子任务:
  - [x] 15.1: 增加霓虹夜航元数据并丰富四套主题描述
  - [x] 15.2: 将 Header 入口改为明确的主题切换双行文案
  - [x] 15.3: 增加赛博朋克深色 Token、网格背景和主要表面映射
  - [x] 15.4: 更新主题定义回归测试
  - [x] 15.5: 执行代码审查、全量测试和四主题浏览器验收

### 任务 16: [x] 增加轻量页面互动特效
- 文件: `src/utils/interactionEffects.js`、`src/components/InteractionEffects.vue`、`tests/interaction-effects.test.js`（新增），`src/components/ThemeSwitcher.vue`、`src/style.css`、`src/App.vue`（修改）
- 依赖: Task 15
- spec 映射: 3.1、3.2、4.1、4.2.1、4.2.2、4.3、5、6、7
- 说明: 使用原生 CSS、DOM 和 View Transition 增加点击成长星火、主题径向涟漪与卡片微光，保持轻量并尊重减少动态效果偏好。
- 验收标准:
  - [x] 已完成的鼠标/触屏点击生成主题色微型烟花，滚动手势与输入控件不触发，720ms 后自动清理
  - [x] 同时最多存在三簇烟花，每簇十个粒子且不拦截页面交互
  - [x] 支持 View Transition 时从操作位置展开主题，不支持时即时切换
  - [x] `prefers-reduced-motion: reduce` 下烟花、涟漪和卡片微光均停用
  - [x] 卡片微光只在 hover 时单次执行，不产生常驻动画
  - [x] 1200 × 800 与 390 × 844 下无横向溢出、滚动阻塞或控制台错误
  - [x] `npm run build` 与 `npm test` 返回 0
  - [x] Code Review PASS
- 子任务:
  - [x] 16.1: 实现可测试的点击粒子模型与生命周期受控的效果层
  - [x] 16.2: 为主题切换增加原生径向 View Transition 与降级路径
  - [x] 16.3: 为主要卡片增加仅 hover 触发的微光反馈
  - [x] 16.4: 增加特效模型边界测试和减少动态效果规则
  - [x] 16.5: 执行代码审查、全量测试与真实浏览器验收

## Spec 覆盖映射

| Spec 章节 | 任务 | 说明 |
|-----------|------|------|
| 3.1 功能性需求 | Task 1、2、3、5、6、12、13、14、15、16 | 覆盖全局、骨架、首页、AI、链接库、管理弹窗、内容 Tab、系统加固、品牌、导航、主题与互动特效 |
| 3.2 非功能性需求 | Task 1、2、3、4、5、6、7、14、15、16 | 覆盖兼容、响应式、离线、性能、动效、安全、可访问性和主题偏好隔离 |
...
| 7.1 单元测试 | Task 6、14、15、16 | 使用 Node 内置测试运行器覆盖安全 URL、迁移、导入、Tauri 识别、并发写入、主题和特效模型 |
| 7.2-7.3 测试计划 | Task 4、5、7、12、13、14、15、16 | 构建、浏览器冒烟、内容边界、Tab 键盘、品牌、导航、四主题与互动特效检查 |
