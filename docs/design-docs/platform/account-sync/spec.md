...
### 1.2 现状分析
- `src/data/store.js` 是分类、链接、孩子档案和成长记录的唯一写入口，以 `schemaVersion + categories + links + growthChildren + growthRecords` 完整快照作为数据模型，并通过串行写队列保证本地一致性。
...
- 兼容当前 Schema v3、宽容 v1/v2 数据迁移、严格导入校验和现有 Store 异步失败语义。
...
#### 4.2.3 数据模型
- 业务快照使用 Schema v3，不混入认证、设备或同步元数据：
  ```ts
  interface Snapshot {
    schemaVersion: 3
    categories: Category[]
...
- 升级兼容：
  ...
  - 当前业务 Snapshot 写入 Schema v3；v1/v2 仅允许新客户端幂等迁移读取，本地封装继续通过 `localFormatVersion` 独立演进。
