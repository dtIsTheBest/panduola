...
### 1.2 现状分析
- `src/data/store.js` 是资源、孩子、成长记录和成长日程的唯一写入口，完整快照新增 `scheduleItems + scheduleCompletions`，并通过串行写队列保证本地一致性。
...
- 兼容当前 Schema v4、宽容 v1-v3 数据迁移、严格导入校验和现有 Store 异步失败语义。
...
#### 4.2.3 数据模型
- 业务快照使用 Schema v4，不混入认证、设备或同步元数据：
  ```ts
  interface Snapshot {
    schemaVersion: 4
    categories: Category[]
    links: Link[]
    growthChildren: GrowthChild[]
...
- 主要优势：
  ...
  - 同步分类、链接、收藏状态、访问次数、孩子档案和成长记录，不扩大到 AI 密钥、对话记录或应用凭据。
- 明确限制：
...
- 升级兼容：
  ...
  - 当前业务 Snapshot 写入 Schema v4；v1-v3 仅允许新客户端幂等迁移读取，本地封装继续通过 `localFormatVersion` 独立演进。
  - 一旦空间或云端写入 v3，不得直接回滚到只支持 v2 的旧客户端；旧客户端遇到更高业务 Schema 必须拒绝写入。
- 回滚：
...
| 2026-08-20 | **Schema v3** | 新增多孩档案与成长记录归属，v1/v2 原始 hash 验证后确定性迁移，未来版本继续 fail-closed |
| 2026-08-20 | **Schema v4** | 新增简约成长日程模板与逐次完成记录，v1-v3 确定性迁移为空集合 |
